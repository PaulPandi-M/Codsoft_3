...
public class Withdraw implements ActionListener{
    ...
    Withdraw(long accnum,int pinnum)
    {
        pinnumber=pinnum;
        accnumber=accnum;
            ...
            public void actionPerformed(ActionEvent ae)
            {
                if(amtpanel.isVisible() && afield.getText().length()>0)
                {
                     ...
                     try{
                         ...
                         if(amount>bal)
                         {
                            JOptionPane.showMessageDialog(wtf,"insufficient amount","Error",JOptionPane.WARNING_MESSAGE);
                            amount=0;
                            afield.setText("");
                         }
                         else
                         ...
                         JOptionPane.showMessageDialog(wtf,"no such account","Error",JOptionPane.ERROR_MESSAGE);
                         afield.setText("");
                ...
                else if(pinpanel.isVisible() && pfield.getText().length()>0)
                {
                    if(encpin==pinnumber)
                    {
                        ...
                        JOptionPane.showMessageDialog(wtf,"incorrect pin", "Error", JOptionPane.ERROR_MESSAGE);
                        encpin=0;
                        pfield.setText("");
                    }
               }
    ...
    void display()
    {
        ...
        wtf.setSize(900,700);
        wtf.setLocationRelativeTo(null);
        wtf.setLayout(null);
        wtf.setResizable(false);
        wtf.addWindowListener(new WindowAdapter() {
            public void windowClosing(WindowEvent we)
            {
                int opt=JOptionPane.showOptionDialog(wtf,"are you sure? you want to exit","exit",JOptionPane.YES_NO_OPTION,JOptionPane.WARNING_MESSAGE,null,null,null);
                if(opt==JOptionPane.YES_OPTION)
                {
                    ...
                    wtf.setDefaultCloseOperation(JFrame.DO_NOTHING_ON_CLOSE);
                }
            }
        });
    ...
    //     Withdraw wd=new Withdraw(1234,1234);
        // wd.createPinpanel();
        // wd.createAmtpanel();
        // wd.numPad();
        // wd.display();
